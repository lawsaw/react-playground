...
class Body extends PureComponent {
    ...
    emitPlayerUpdate = (callback, action) => {
        const { user } = this.state;
    ...
    handleLeaveRoom = () => {
        this.emitRoomAccess(null, 'LEAVE');
        //this.emitPlayerUpdate(null, 'LEAVE');
    }

    emitRoomAccess = (room, event, props) => {
        this.emit(EMIT_ROOM_ACCESS, {
            room,
            event,
            ...props
        });
    ...
    render() {
        const { image, user, lobbyStep, roomList, room, isNewRoomCreateModal, newRoomName, chat } = this.state;
